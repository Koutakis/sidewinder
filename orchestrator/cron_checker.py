...
class CronChecker:
    ...
    def check_and_start(self, force: bool = False) -> None:
        if force:
            print(f"✓ Starting {self.model_name} (forced run)")
            return
        ...
        if now >= next_run:
            ...
            print(
                f"⏭  {self.model_name} not due yet (last ran {last_run}, next run {next_run})"
            )
            sys.exit(0)
