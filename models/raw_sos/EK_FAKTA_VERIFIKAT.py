...
config = Model(
    ...
    columns=[
        ...
        PostgresColumn(name="DAVST_V", data_type=PostgresType.NUMERIC),
        ...
        PostgresColumn(name="KAVST_V", data_type=PostgresType.NUMERIC),
        ...
        PostgresColumn(name="RAPPD_ID", data_type=PostgresType.TEXT),
...
def execute(env: EnvConfig, cfg=config):
	...
	[VERTYP] AS VERTYP,
	[YRKG_ID] AS YRKG_ID
    FROM [raindance_udp].[udp_220].[EK_FAKTA_VERIFIKAT]
    WHERE CAST(VERDATUM AS DATE) BETWEEN '{since}' AND '{until}'
    """
    total_rows = 0
    first_batch = True
    for df in read("RAINDANCE_8570", query, batch_size=500_000):
        if len(df) == 0:
            continue
