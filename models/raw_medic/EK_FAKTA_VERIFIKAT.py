...
config = Model(
    ...
    columns=[
        ...
        PostgresColumn(name="PROGN_V", data_type=PostgresType.NUMERIC),
        ...
        PostgresColumn(name="TRAKOD_ID", data_type=PostgresType.TEXT),
...
def execute(env: EnvConfig, cfg=config):
	...
	[VERRAD] AS VERRAD,
	[VERTYP] AS VERTYP
    FROM [MediCarrierUDP].[utdata100].[EK_FAKTA_VERIFIKAT]
    WHERE CAST(VERDATUM AS DATE) BETWEEN '{since}' AND '{until}'
    """
    total_rows = 0
    first_batch = True
    for df in read("RAINDANCE_8090", query, batch_size=500_000):
        if len(df) == 0:
            continue
