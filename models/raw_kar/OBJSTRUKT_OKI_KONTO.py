...
config = Model(
    name="objstrukt_oki_konto",
    source_entity="OBJSTRUKT_OKI_KONTO",
    table="objstrukt_oki_konto",
    ...
    columns=[
        ...
        PostgresColumn(name="OBJ_OKI", data_type=PostgresType.TEXT),
    ],
...
def execute(env, cfg=config):
    query=f"""SELECT * FROM (SELECT
    	...
    	[OBJ_OKI] AS OBJ_OKI
    FROM [Utdata].[udp_100].[OBJSTRUKT_OKI_KONTO]) y
    WHERE 1=1"""
