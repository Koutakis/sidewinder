...
config = Model(
    name="objstrukt_defanl_fs",
    source_entity="OBJSTRUKT_DEFANL_FS",
    table="objstrukt_defanl_fs",
    ...
    columns=[
        ...
        PostgresColumn(name="OBJ_FS", data_type=PostgresType.TEXT),
    ],
...
def execute(env, cfg=config):
    query=f"""SELECT * FROM (SELECT
    	...
    	[OBJ_FS] AS OBJ_FS
    FROM [raindance_udp].[udp_100].[OBJSTRUKT_DEFANL_FS]) y
    WHERE 1=1"""
