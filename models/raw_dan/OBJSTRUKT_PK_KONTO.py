...
config = Model(
    name="objstrukt_pk_konto",
    source_entity="OBJSTRUKT_PK_KONTO",
    table="objstrukt_pk_konto",
    ...
    columns=[
        ...
        PostgresColumn(name="OBJ_PK", data_type=PostgresType.TEXT),
    ],
...
def execute(env, cfg=config):
    query=f"""SELECT * FROM (SELECT
    	...
    	[OBJ_PK] AS OBJ_PK
    FROM [raindance_udp].[udp_150].[OBJSTRUKT_PK_KONTO]) y
    WHERE 1=1"""
