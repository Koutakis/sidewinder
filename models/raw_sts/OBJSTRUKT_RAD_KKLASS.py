...
config = Model(
    name="objstrukt_rad_kklass",
    source_entity="OBJSTRUKT_RAD_KKLASS",
    table="objstrukt_rad_kklass",
    ...
    columns=[
        ...
        PostgresColumn(name="OBJ_RAD", data_type=PostgresType.TEXT),
    ],
...
def execute(env, cfg=config):
    query=f"""SELECT * FROM (SELECT
    	...
    	[OBJ_RAD] AS OBJ_RAD
    FROM [stsudp].[udp_858].[OBJSTRUKT_RAD_KKLASS]) y
    WHERE 1=1"""
